...
mt-enfr-lstm/
│
├── data/                         # Dữ liệu song ngữ Multi30K (EN–FR)
│
├── enfr-main/
│   └── src/
│	└── checkpoints/
│       |   ├──best_seq2seq.pth   # Lưu trên kaggle
│       └── main/
│           ├── dataset.py        # Tokenization, Vocabulary, DataLoader
│           ├── model.py          # Encoder–Decoder LSTM (Seq2Seq)
│           ├── train.py          # Huấn luyện mô hình + lưu checkpoint
│           ├── evaluate.py       # Đánh giá mô hình (BLEU score)
│           ├── inference.py      # Dịch câu (Greedy decoding)
│           ├── loadCheckpoint.py # Nạp mô hình đã huấn luyện
│           └── main.py            # File chạy chính (entry point)
│
├── notebooks/                    # Notebook phân tích & thực nghiệm
│   ├── test2016/                 # Thực nghiệm trên tập test2016
│   ├── test2017/                 # Thực nghiệm trên tập test2017
│   └── test2018/                 # Thực nghiệm trên tập test2018
│
├── .gitignore                    # Loại trừ file tạm, checkpoint, env
